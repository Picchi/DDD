...
size_t WriteImg(char *ptr, size_t size, size_t nmemb, void *userdata)
{
	...
	double d;
	unsigned long dd; 
	struct __WriteImg *data=(struct __WriteImg *)userdata;
	curl_easy_getinfo(data->curl,CURLINFO_SIZE_DOWNLOAD, &d);
	curl_easy_getinfo(data->curl,CURLINFO_REQUEST_SIZE, &dd);
	//CURLINFO_SIZE_DOWNLOAD
	//CURLINFO_REQUEST_SIZE
	printf("%f/%lu\r",d,dd );
	return fwrite(ptr,size,nmemb,data->f);
	*/
...
wchar_t* fromUTF8toUTF16 (char * U8)
{
	...
	while (ptr<dimU8){
		...
		if (U16==NULL){
			perror("Error realloc fromUTF8toUTF16");
			return NULL;
		}
		U16[dim]='\0';
...
int StartImg(int NumPage,char ** P, char **H,char*LinkPage,CURL * curl,struct Mem *Page,
						 wchar_t* WTitle,uint8_t NumImg,uint8_t *LastPage)
{
	int i,g;
	//char NameImg[5];
	...
	for (i=1;i<=NumPage;i++){
		...
		if (h==INVALID_HANDLE_VALUE ){
			...
			if (i>NumImg){
				...
				sprintf(supp,"%s?p=%d",LinkPage,(*LastPage));
				//printf("supp %s\n",supp);
				free(Page->mem);
				//Page->mem=malloc(1);
				Page->mem=NULL;
...
char *GetNextPage(unsigned int *i,char * End,int NumPage,char * LinkPage,CURL * curl,wchar_t *WTitle,
				  struct Mem * Page,uint8_t NumImg,char **H,uint8_t*LastPage)
{
	int g,j;
	char Sapp[20],supp[200];
	wchar_t WNameImg[256];
	...
	//(*i)++;
	for (j=(*i)+1;;j++){
		swprintf(WNameImg,L"%s\\%0*d.*",WTitle,g,j);
		//printf("%s\n",NameImg );
		h=FindFirstFileW(WNameImg,&d);
		if (h==INVALID_HANDLE_VALUE ){
			//printf("find for %d/%d\n",j,NumPage );
			...
			//printf("find %d %s\n",i,d.cFileName );
			sprintf(Sapp,"\"><img alt=\"%0*d",g,j);
			//if (j>NumImg){
			if((*LastPage)<j/NumImg){
				(*LastPage)=j/NumImg;
				//printf("LinkPage %s \n",LinkPage);
				if(j%NumImg==0){
					(*LastPage)--;
				...
				//printf("ff GetNextPage%p\n",*H );
				free(*H);
				//printf("fff\n");
				*H=malloc(Page->size+1);
				if(*H==NULL){
					perror("malloc GetNextPage");
					exit(1);
				}
				//printf("ffff\n");
				(*H)[Page->size]='\0';	
...
void  GetGallery(struct Mem *HP,char *LinkPage ,CURL * curl,
				...
				int NumThread,unsigned long int TimeOut)
{
...
//	if (!CreateDirectory(Title,NULL)){
	...
	curl_easy_setopt(CurlGal,CURLOPT_TIMEOUT,TimeOut);
	//printf("start %p\n",(*HP).mem);
	for (i=StartImg(NumPage,&P,&((*HP).mem),LinkPage,curl,&Page,WTitle,NumImg,&LastPage);i<=NumPage;){
		//printf("HP.mem %p\n",(*HP).mem );
		//printf("i for:%d\n", i);
		//printf("HH %s\n",HP.mem );
		//printf("i:%d\n",i);
	...
	try:
		...
		do {
			if(Verbose && NumThread==1){
				printf("Get page %s %0*d/%d\r",Title,g,i,NumPage);
			}
			res = curl_easy_perform(curl);
			if(res != CURLE_OK){
	...
	try2:
		if(Verbose && NumThread==1){
			printf("Get img %s %0*d/%d \r",Title,g,i,NumPage);
		}
	...
	*/
	...
	for (i=0;i<par->NumGallery;i++){
		...
		GetGallery(&HP,(par->LinkPage)[i],curl,par->proxy,par->ProxyNum,
				   ...
				   par->ExcludeTag,par->NumExcludeTag,par->NumImg,par->NumThread,par->TimeOut);
		//printf("HP.mem %p\n",HP.mem );
							   ...
							   unsigned long int TimeOut)
{
	...
	for (i=0;i<NumThread;i++){
		...
		ret[i].NumThread=NumThread;
		ret[i].TimeOut=TimeOut;
	}
...
int main (int argc,char **argv)
{
	...
	//if (strrchr(argv[0],'\\')!=NULL){
		 GetLinkReverse(cookie,"\\",NULL,NULL,&Pname);
	//} else {
	//	Pname= argv[0];
	//}
	//printf("%s\n",Pname );
...
//	printf("Ciao\n");
	...
	*/
	...
	param=SepareteGallery(NumThread,optind,Cargc,Cargv,proxy,ProxyNum,Verbose,ExcludeTag,NumExcludeTag,Type+4,TimeOut);
